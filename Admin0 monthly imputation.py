...
def clean_country_name(country_name):
    """Remove trailing frequency labels from a country name."""
    return re.sub(r"(?:\s+(?:monthly|yearly))+$", "", country_name, flags=re.IGNORECASE).strip()


def find_source_tifs(month_folder_path):
    """Map each nested source-folder series to its single GeoTIFF for a month."""
    if not month_folder_path or not os.path.isdir(month_folder_path):
        return {}

    tif_paths = []
    for root, dir_names, file_names in os.walk(month_folder_path):
        ...
        for file_name in sorted(file_names):
            ...
            if (
                ...
                tif_paths.append(os.path.join(root, file_name))

    source_series = {}
    for tif_path in sorted(tif_paths):
        relative_parent = os.path.relpath(os.path.dirname(tif_path), month_folder_path)
        series_key = relative_parent.replace(os.sep, "/").casefold()
        if series_key in source_series:
            print(
                f"[Warning] Multiple GeoTIFFs found in source series {relative_parent} "
                f"under {month_folder_path}; skipping {os.path.basename(tif_path)}"
            )
            continue
        source_series[series_key] = (tif_path, relative_parent)
...
def find_json_sidecar(tif_path, month_folder_path):
    """Find the TIFF's JSON sidecar between its directory and the month directory."""
    direct_match = os.path.splitext(tif_path)[0] + ".json"
    if os.path.isfile(direct_match):
        return direct_match

    current_dir = os.path.dirname(tif_path)
    month_dir = os.path.abspath(month_folder_path)
    while True:
        json_paths = sorted(
        ...
        if len(json_paths) == 1:
            return json_paths[0]
        if os.path.abspath(current_dir) == month_dir:
            return None
...
def update_json_statistics(tif_path, month_folder_path, original_arr, repaired_arr, repair_counts):
    """Store only the requested repair statistics in the JSON sidecar."""
    json_path = find_json_sidecar(tif_path, month_folder_path)
    ...
    try:
        ...
        document["stats"] = {
            ...
            "nan_percentage_repaired": float(
                (np.count_nonzero(original_nan) - np.count_nonzero(repaired_nan)) / total_pixels * 100.0
            ),
        }
...
def build_country_month_summary_dataframe(country_dir, country):
    """Build one country's monthly summary from the repaired source rasters."""
    columns = [
        "country",
        "year",
        "month",
    ...
    rows = []
    years = sorted(
        (name for name in os.listdir(country_dir) if name.isdigit() and os.path.isdir(os.path.join(country_dir, name))),
        key=int,
    )

    for year in years:
        for month, month_folder_path, _ in get_month_dirs(os.path.join(country_dir, year)):
            values = []
            nan_pixels = 0
            total_pixels = 0
            repaired_pixels = 0
            shape_values = []
            sources = find_source_tifs(month_folder_path)
            for tif_path, _ in sources.values():
                array, _ = load_tif_safe(tif_path)
                if array is not None:
                    valid_values = array[np.isfinite(array) & (array > 0.0)]
                    nan_pixels += int(np.count_nonzero(~np.isfinite(array) | (array <= 0.0)))
                ...
                try:
                    with open(json_path, "r", encoding="utf-8") as handle:
                        document = json.load(handle)
                    repaired_pixels += int(document.get("stats", {}).get("repaired_pixels", 0))
                    ...
                    print(f"[Warning] Could not read repaired pixel count ({json_path}): {error}")

            all_values = np.concatenate(values) if values else np.array([], dtype=np.float32)
            unique_shapes = list(dict.fromkeys(shape_values))
            rows.append(
                {
                    "country": country,
                    "year": int(year),
                    ...
                    "shape": (
                        ...
                        else json.dumps(unique_shapes)
                        if unique_shapes
                        else None
                    ),
                    "min": float(np.min(all_values)) if all_values.size else None,
                    "max": float(np.max(all_values)) if all_values.size else None,
                    "mean": float(np.mean(all_values)) if all_values.size else None,
                    "median": float(np.median(all_values)) if all_values.size else None,
                    "std": float(np.std(all_values)) if all_values.size else None,
                    "nan_percentage_before": (
                        (nan_pixels + repaired_pixels) / total_pixels * 100.0 if total_pixels else None
                    ),
                    "nan_percentage": nan_pixels / total_pixels * 100.0 if total_pixels else None,
                    "repaired_pixels": repaired_pixels,
                }
            )
...
def build_all_countries_month_summary_csv(country_summaries):
    """Write all country-month summaries to one CSV beside this script."""
    if not country_summaries:
        print("[Warning] No country summaries were available for the combined CSV.")
        return None

    summary_path = os.path.join(script_dir, "Admin0_monthly_summary.csv")
    pd.concat(country_summaries, ignore_index=True).sort_values(
        ["country", "year", "month"], na_position="last"
    ).to_csv(summary_path, index=False)
    print(f"  -> Wrote combined country-month summary to {summary_path}")
    return summary_path
...
def main():
    ...
    requested_country = country_key(clean_country_name(args.country)) if args.country else None
    overall = {
        "rasters": 0,
        "missing_pixels": 0,
        "repaired_pixels": 0,
    ...
    for folder in sorted(os.listdir(input_dir)):
        ...
        country = clean_country_name(folder)
        ...
        def get_raw_raster(year, month, series_key):
            ...
            if key not in raw_cache:
                month_folder_path = month_dirs.get(year, {}).get(month)
                source = source_series.get(year, {}).get(month, {}).get(series_key)
                if source is None:
                    raw_cache[key] = None
                else:
                    tif_path, series_label = source
                    array, profile = load_tif_safe(tif_path)
                    raw_cache[key] = (
                        (tif_path, month_folder_path, array, profile, series_label)
        ...
        for year, month, series_key in target_records:
                ...
                tif_path, month_folder_path, target_arr, target_profile, series_label = target
                overall["rasters"] += 1
        ...
        if not args.dry_run:
            country_summaries.append(build_country_month_summary_dataframe(folder_path, country))
    ...
    if not args.dry_run:
        build_all_countries_month_summary_csv(country_summaries)
